    ...
    <div className="overflow-x-auto">
      <table className="w-full text-left text-sm">
        <thead className="text-[10px] text-neutral-500 uppercase border-b border-neutral-800">
          <tr><th className="pb-2 font-bold pl-2">Card</th><th className="pb-2 font-bold text-right">Limit</th><th className="pb-2 font-bold text-right">Spent</th><th className="pb-2 font-bold text-right pr-2">Avail</th></tr>
        </thead>
        <tbody className="text-neutral-300">
          {cards.map(c => {
              const limit = c.manual_limit || c.total_limit;
              return (
                <tr key={c.id} className="border-b border-neutral-800/50 hover:bg-neutral-800/30">
                  ...
                  <td className="py-3 text-right pr-2 text-green-500">{currency} {(limit - c.spent).toLocaleString()}</td>
                </tr>
              );
          })}
        </tbody>
...
const TransactionsModal = ({ onClose, currency }) => {
    ...
    const handleUpdate = async (e) => {
        ...
        await axios.put(`${API_URL}/transactions/${editingTxn.id}`, {
            description: editingTxn.description, amount: parseFloat(editingTxn.amount), date: new Date(editingTxn.date).toISOString()
        }, { headers: { Authorization: `Bearer ${token}` } });
        setTransactions(prev => prev.map(t => t.id === editingTxn.id ? {...t, ...editingTxn, amount: parseFloat(editingTxn.amount)} : t));
        setEditingTxn(null);
    };
            ...
            {editingTxn ? (
                <form onSubmit={handleUpdate} className="flex flex-col gap-4 bg-neutral-950 p-4 rounded-xl border border-neutral-800">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <FormField label="Amount"><Input type="number" step="0.01" value={editingTxn.amount} onChange={e => setEditingTxn({...editingTxn, amount: e.target.value})} /></FormField>
                        <FormField label="Date"><Input type="date" value={new Date(editingTxn.date).toISOString().split('T')[0]} onChange={e => setEditingTxn({...editingTxn, date: e.target.value})} /></FormField>
                    </div>
                    <div className="flex gap-2 justify-end pt-2 border-t border-neutral-800/50">
                        <button type="button" onClick={() => setEditingTxn(null)} className="bg-neutral-800 text-white px-4 py-3 rounded-xl text-xs font-bold">Cancel</button>
                        <button type="submit" className="bg-red-600 text-white px-6 py-3 rounded-xl text-xs font-bold">Save</button>
                    </div>
                </form>
            ) : (
                    ...
                    {loading ? <p className="text-center text-neutral-500 py-4">Loading...</p> : 
                     transactions.map(t => (
                        <div key={t.id} className="bg-neutral-950 p-4 rounded-xl border border-neutral-800 flex justify-between items-center">
                            <div><p className="text-white font-medium text-sm">{t.description}</p><p className="text-[10px] text-neutral-500">{formatDate(t.date)} • {t.mode}</p></div>
                            <div className="text-right"><p className="text-white font-bold text-sm">{currency} {t.amount.toLocaleString()}</p>
...
const EditCardModal = ({ card, onClose, onDelete, onUpdate }) => {
  ...
  useEffect(() => {
    if (tab === 'statements') {
        const token = localStorage.getItem('token');
        axios.get(`${API_URL}/cards/${card.id}/statements`, { headers: { Authorization: `Bearer ${token}` } })
             .then(res => setStatements(res.data)).catch(console.error);
    }
  }, [tab, card.id]);
  ...
  const handleStmtSubmit = async (e) => {
      e.preventDefault();
      const token = localStorage.getItem('token');
      await axios.post(`${API_URL}/cards/${card.id}/statements`, { date: new Date(newStmt.date).toISOString(), amount: parseFloat(newStmt.amount), card_id: card.id }, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const deleteStmt = async (id) => {
      ...
      const token = localStorage.getItem('token');
      await axios.delete(`${API_URL}/cards/${card.id}/statements/${id}`, { headers: { Authorization: `Bearer ${token}` } });
      setStatements(prev => prev.filter(s => s.id !== id));
  };
  
  const togglePaid = async (stmt) => {
      const token = localStorage.getItem('token');
      await axios.put(`${API_URL}/cards/${card.id}/statements/${stmt.id}`, { is_paid: !stmt.is_paid }, { headers: { Authorization: `Bearer ${token}` } });
      setStatements(prev => prev.map(s => s.id === stmt.id ? {...s, is_paid: !s.is_paid} : s));
  };

  return (
    <Modal title={`Manage ${card.name}`} onClose={onClose}>
      <div className="flex gap-2 mb-6 border-b border-neutral-800 pb-2 overflow-x-auto">
        {['view', 'details', 'images', 'statements'].map(t => (
            <button key={t} onClick={() => setTab(t)} className={`flex-none px-4 pb-2 text-sm font-medium capitalize ${tab===t ? 'text-red-500 border-b-2 border-red-500' : 'text-neutral-400'}`}>{t}</button>
        ))}
      </div>

      {tab === 'view' && (
          <div className="flex flex-col items-center gap-6 py-4">
               <div className="w-full h-48 bg-gradient-to-br from-neutral-800 to-black rounded-2xl p-6 flex flex-col justify-between shadow-2xl border border-neutral-700">
                  ...
                  <div className="text-white text-xl font-mono tracking-widest text-center mt-2">{formData.full_number || `•••• •••• •••• ${formData.last_4 || '0000'}`}</div>
                  <div className="flex justify-between items-end">
                      <div><p className="text-[8px] text-white/50 uppercase tracking-wide mb-1">Card Holder</p><p className="text-sm text-white font-medium uppercase">{formData.card_holder || 'CARD HOLDER'}</p></div>
                      <div className="text-right"><p className="text-[8px] text-white/50 uppercase tracking-wide mb-1">Valid Thru</p><p className="text-sm text-white font-mono">{formData.valid_thru || 'MM/YY'}</p></div>
                  </div>
              </div>
          </div>
      )}
      ...
      {tab === 'details' && (
        <div className="space-y-6">
           <div className="grid grid-cols-2 gap-4">
               <FormField label="Nickname"><Input value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} disabled={!isEditing} /></FormField>
               <FormField label="Bank"><Input value={formData.bank} onChange={e => setFormData({...formData, bank: e.target.value})} disabled={!isEditing} /></FormField>
           </div>
           <FormField label="Card Holder"><Input value={formData.card_holder || ''} onChange={e => setFormData({...formData, card_holder: e.target.value})} disabled={!isEditing} /></FormField>
           <div className="grid grid-cols-2 gap-4">
              <FormField label="Limit"><Input type="number" value={formData.total_limit} onChange={e => setFormData({...formData, total_limit: e.target.value})} disabled={!isEditing} /></FormField>
              <FormField label="Manual Limit"><Input type="number" value={formData.manual_limit || ''} onChange={e => setFormData({...formData, manual_limit: e.target.value})} disabled={!isEditing} /></FormField>
           </div>
           <div className="grid grid-cols-2 gap-4">
              <FormField label="Statement Day"><Select value={formData.statement_date} onChange={e => setFormData({...formData, statement_date: e.target.value})} disabled={!isEditing}>{[...Array(31)].map((_, i) => <option key={i} value={i+1}>{i+1}th</option>)}</Select></FormField>
              <FormField label="Due Day"><Select value={formData.payment_due_date} onChange={e => setFormData({...formData, payment_due_date: e.target.value})} disabled={!isEditing}>{[...Array(31)].map((_, i) => <option key={i} value={i+1}>{i+1}th</option>)}</Select></FormField>
           </div>
           {isEditing ? (
              <div className="flex gap-3 pt-4">
                  <button onClick={() => setIsEditing(false)} className="flex-1 bg-neutral-800 text-white py-3.5 rounded-xl font-bold">Cancel</button>
                  <button onClick={handleUpdateCard} className="flex-1 bg-red-600 text-white py-3.5 rounded-xl font-bold">Save</button>
              </div>
           ) : (
              <>
                 <button onClick={() => setIsEditing(true)} className="w-full bg-white text-black py-3.5 rounded-xl font-bold flex items-center justify-center gap-2"><Edit2 size={16}/> Edit Details</button>
                 <button onClick={() => onDelete(card.id)} className="w-full border border-red-900/30 bg-red-900/10 text-red-500 py-3.5 rounded-xl font-bold flex items-center justify-center gap-2"><Trash2 size={16}/> Delete Card</button>
              </>
           )}
        </div>
      )}
      ...
      {tab === 'images' && (
        <div className="space-y-6">
           <div className="space-y-2"><label className="text-xs text-neutral-500 font-bold">Front</label>{formData.image_front ? <img src={formData.image_front} className="w-full rounded-xl border border-neutral-700"/> : <div className="h-32 border-2 border-dashed border-neutral-800 rounded-xl flex items-center justify-center text-neutral-600">No Image</div>}</div>
           <div className="space-y-2"><label className="text-xs text-neutral-500 font-bold">Back</label>{formData.image_back ? <img src={formData.image_back} className="w-full rounded-xl border border-neutral-700"/> : <div className="h-32 border-2 border-dashed border-neutral-800 rounded-xl flex items-center justify-center text-neutral-600">No Image</div>}</div>
        </div>
      )}

      {tab === 'statements' && (
          <div className="space-y-6">
              <form onSubmit={handleStmtSubmit} className="flex flex-col gap-4 bg-neutral-950 p-4 rounded-xl border border-neutral-800">
                  <div className="grid grid-cols-2 gap-4">
                      <FormField label="Date"><Input type="date" value={newStmt.date} onChange={e => setNewStmt({...newStmt, date: e.target.value})} required /></FormField>
                      <FormField label="Amount"><Input type="number" step="0.01" value={newStmt.amount} onChange={e => setNewStmt({...newStmt, amount: e.target.value})} required /></FormField>
                  </div>
                  <button className="bg-red-600 text-white px-4 py-3 rounded-xl text-xs font-bold w-full">Add Statement</button>
              </form>
              <div className="max-h-[300px] overflow-y-auto space-y-2">
                  {statements.map(s => (
                      <div key={s.id} className="flex justify-between items-center p-4 rounded-xl bg-neutral-900 border border-neutral-800">
                          <div className="flex items-center gap-4">
                              <button onClick={() => togglePaid(s)} className={`p-2 rounded-full ${s.is_paid ? 'text-green-500 bg-green-900/20' : 'text-neutral-600 bg-neutral-950 border border-neutral-700'}`}>{s.is_paid ? <CheckCircle size={20}/> : <div className="w-5 h-5 rounded-full"/>}</button>
                              <div><span className="text-sm text-neutral-300 block font-medium">{formatDate(s.date)}</span>{s.is_paid && <span className="text-[10px] text-green-500 font-bold">Paid</span>}</div>
                          </div>
                          <div className="flex items-center gap-3">
                              <span className="font-bold text-lg text-white">{parseFloat(s.amount).toLocaleString()}</span>
                              <button onClick={() => deleteStmt(s.id)} className="text-neutral-600 hover:text-red-500"><Trash2 size={16}/></button>
                          </div>
                      </div>
                  ))}
                  {statements.length === 0 && <p className="text-center text-neutral-500 py-4">No statements.</p>}
              </div>
          </div>
      )}
    </Modal>
...
const SearchPage = ({ currency, privacy }) => {
    ...
    const handleSearch = async (e) => {
        ...
        Object.keys(filters).forEach(key => { if(filters[key]) params.append(key, key.includes('date') ? new Date(filters[key]).toISOString() : filters[key]); });
    ...
    return (
        <div className="space-y-6 animate-in fade-in">
            ...
            <form onSubmit={handleSearch} className="space-y-4 bg-neutral-900 p-4 rounded-xl border border-neutral-800">
                <FormField label="Keyword"><Input value={filters.search} onChange={e=>setFilters({...filters, search:e.target.value})}/></FormField>
                <div className="grid grid-cols-2 gap-4"><FormField label="Min"><Input type="number" value={filters.min_amount} onChange={e=>setFilters({...filters, min_amount:e.target.value})}/></FormField><FormField label="Max"><Input type="number" value={filters.max_amount} onChange={e=>setFilters({...filters, max_amount:e.target.value})}/></FormField></div>
                <div className="grid grid-cols-2 gap-4"><FormField label="Start"><Input type="date" value={filters.start_date} onChange={e=>setFilters({...filters, start_date:e.target.value})}/></FormField><FormField label="End"><Input type="date" value={filters.end_date} onChange={e=>setFilters({...filters, end_date:e.target.value})}/></FormField></div>
                <button className="w-full bg-blue-600 text-white py-3.5 rounded-xl font-bold mt-2">Search</button>
            </form>
            <div className="space-y-2">{results.map(t => (<div key={t.id} className="bg-neutral-900 p-4 rounded-xl border border-neutral-800 flex justify-between items-center"><div><p className="text-white font-medium text-sm">{t.description}</p><p className="text-[10px] text-neutral-500">{formatDate(t.date)}</p></div><p className="text-white font-bold text-sm">{formatMoney(t.amount, currency, privacy)}</p></div>))}</div>
        </div>
    );
};
...
const SubscriptionsPage = ({ currency, privacy }) => {
    ...
    const fetchSubs = async () => { const res = await axios.get(`${API_URL}/subscriptions/`, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }); setSubs(res.data); };
    useEffect(() => { fetchSubs(); }, []);

    const handleAdd = async (e) => {
        e.preventDefault(); 
        await axios.post(`${API_URL}/subscriptions/`, { ...newSub, amount: parseFloat(newSub.amount), next_due_date: new Date(newSub.next_due_date).toISOString() }, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } });
        setShowAdd(false); fetchSubs();
    };

    const handleDelete = async (id) => {
        if(!confirm("Stop tracking?")) return;
        await axios.delete(`${API_URL}/subscriptions/${id}`, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } });
        fetchSubs();
    };
    
    const handleFile = async (e) => { try { const b64 = await processImage(e.target.files[0]); setNewSub(prev => ({...prev, attachment: b64})); } catch(e) { alert("Error"); } };

    return (
        <div className="space-y-6 animate-in fade-in">
            <div className="flex justify-between items-center"><h2 className="text-2xl font-bold text-white">Subscriptions</h2><button onClick={() => setShowAdd(true)} className="bg-neutral-800 text-white p-2 rounded-lg border border-neutral-700"><Plus size={20}/></button></div>
            <div className="grid gap-3">{subs.map(s => (<div key={s.id} className="bg-neutral-900 p-4 rounded-xl border border-neutral-800 flex justify-between items-center"><div className="flex items-center gap-3"><div className="bg-purple-500/20 p-2.5 rounded-lg text-purple-400"><RefreshCw size={18}/></div> <div><p className="text-white font-bold text-sm">{s.name}</p><p className="text-[10px] text-neutral-500">Next: {formatDate(s.next_due_date)} • {s.billing_cycle}</p></div></div><div className="flex items-center gap-3">{s.attachment && <button onClick={() => setViewAtt(s.attachment)} className="text-neutral-500"><Eye size={16}/></button>}<p className="text-white font-bold text-sm">{formatMoney(s.amount, currency, privacy)}</p><button onClick={() => handleDelete(s.id)} className="text-[10px] text-red-500 hover:underline">Remove</button></div></div>))}</div>
            {showAdd && <Modal title="Add Subscription" onClose={() => setShowAdd(false)}><form onSubmit={handleAdd} className="space-y-4"><FormField label="Name"><Input value={newSub.name} onChange={e=>setNewSub({...newSub, name: e.target.value})} required/></FormField><div className="grid grid-cols-2 gap-4"><FormField label="Amount"><Input type="number" value={newSub.amount} onChange={e=>setNewSub({...newSub, amount: e.target.value})} required/></FormField><FormField label="Cycle"><Select value={newSub.billing_cycle} onChange={e=>setNewSub({...newSub, billing_cycle: e.target.value})}><option>Monthly</option><option>Yearly</option></Select></FormField></div><FormField label="Next Date"><Input type="date" value={newSub.next_due_date} onChange={e=>setNewSub({...newSub, next_due_date: e.target.value})} required/></FormField><FormField label="Attachment"><div className="border-2 border-dashed border-neutral-700 rounded-xl p-4 text-center cursor-pointer" onClick={() => fileRef.current.click()}><Upload className="mx-auto mb-2 text-neutral-500"/><span className="text-xs text-neutral-400">{newSub.attachment ? 'File Selected' : 'Upload PDF/Img'}</span></div><input type="file" ref={fileRef} accept="image/*,.pdf" className="hidden" onChange={handleFile}/></FormField><button className="w-full bg-purple-600 text-white py-3.5 rounded-xl font-bold mt-2">Track</button></form></Modal>}
            {viewAtt && <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/95 p-4" onClick={() => setViewAtt(null)}><button onClick={() => setViewAtt(null)} className="absolute top-6 right-6 bg-neutral-800 text-white p-3 rounded-full z-[80]"><X/></button>{viewAtt.startsWith('data:application/pdf') ? <iframe src={viewAtt} className="w-full h-[85vh] rounded-lg border-none"/> : <img src={viewAtt} className="max-w-full max-h-[85vh] object-contain"/>}</div>}
        </div>
    );
};

const CalendarPage = ({ cards }) => {
    const today = new Date();
    const daysInMonth = new Date(today.getFullYear(), today.getMonth()+1, 0).getDate();
    const startDay = new Date(today.getFullYear(), today.getMonth(), 1).getDay(); 
    const events = [];
    cards.forEach(c => { events.push({ day: c.payment_due_date, type: 'due' }); events.push({ day: c.statement_date, type: 'stmt' }); });
...
const LendingPage = ({ currentUser, privacy }) => {
    ...
    const fetchData = async () => { const res = await axios.get(`${API_URL}/lending/`, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }); setList(res.data); };
    useEffect(() => { fetchData(); }, []);
    ...
    const handleAddOrUpdate = async (e) => { 
        ...
        try {
            if(showEdit && options) await axios.put(`${API_URL}/lending/${options.id}`, payload, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } });
            else await axios.post(`${API_URL}/lending/`, payload, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } });
            setShowAdd(false); setShowEdit(false); setOptions(null); fetchData();
            setNewItem({ borrower_name: '', amount: '', lent_date: new Date().toISOString().split('T')[0], reminder_date: '', attachment_lent: '' });
        } catch(err) { alert("Failed"); }
    };
    
    const handleDelete = async (id) => { if(!confirm("Delete?")) return; await axios.delete(`${API_URL}/lending/${id}`, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }); setOptions(null); fetchData(); };
    const handleReturn = async (e) => { e.preventDefault(); await axios.put(`${API_URL}/lending/${showReturn}/return`, { is_returned: true, returned_date: new Date(returnItem.returned_date).toISOString(), attachment_returned: returnItem.attachment_returned }, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }); setShowReturn(null); fetchData(); };
    const handleFile = async (e, setter, field) => { try { const b64 = await processImage(e.target.files[0]); setter(prev => ({...prev, [field]: b64})); } catch(e) { alert("Error"); } };
    
    const handleTouchStart = (item) => { longPressTimer.current = setTimeout(() => { if(navigator.vibrate) navigator.vibrate(50); setOptions(item); }, 500); };
    const handleTouchEnd = () => { if(longPressTimer.current) clearTimeout(longPressTimer.current); };
    const startEdit = (item) => { setNewItem({ borrower_name: item.borrower_name, amount: item.amount, lent_date: new Date(item.lent_date).toISOString().split('T')[0], reminder_date: item.reminder_date ? new Date(item.reminder_date).toISOString().split('T')[0] : '', attachment_lent: item.attachment_lent }); setOptions(item); setShowEdit(true); };

    return (
        <div className="space-y-6 animate-in fade-in relative">
            {options && !showEdit && (<div className="fixed inset-0 z-50 flex items-center justify-center animate-in fade-in duration-200"><div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => setOptions(null)}></div><div className="bg-neutral-900 border border-neutral-700 p-1 rounded-2xl w-3/4 max-w-[200px] flex flex-col gap-1 z-50"><div className="text-center py-3 text-xs font-bold text-neutral-400 uppercase border-b border-neutral-800">Options</div><button onClick={() => startEdit(options)} className="w-full bg-neutral-800 text-white p-3 rounded-xl flex items-center gap-3 hover:bg-neutral-700"><Edit2 size={16}/><span className="font-medium text-sm">Edit</span></button><button onClick={() => handleDelete(options.id)} className="w-full bg-neutral-800 text-red-400 p-3 rounded-xl flex items-center gap-3 hover:bg-red-900/20"><Trash2 size={16}/><span className="font-medium text-sm">Delete</span></button><button onClick={() => setOptions(null)} className="w-full text-neutral-500 text-sm py-3 hover:text-white">Cancel</button></div></div>)}
            <div className="flex justify-between items-center"><h2 className="text-2xl font-bold text-white">Debt Portfolio</h2><button onClick={() => setShowAdd(true)} className="bg-neutral-800 text-white px-4 py-2 rounded-xl flex items-center gap-2 text-sm font-bold border border-neutral-700"><Plus size={16}/> Lend Money</button></div>
            <div className="space-y-3">{list.map(item => (<div key={item.id} className="p-4 rounded-xl border bg-neutral-900 border-neutral-800 select-none active:scale-95 transition-transform" onTouchStart={() => handleTouchStart(item)} onTouchEnd={handleTouchEnd} onMouseDown={() => handleTouchStart(item)} onMouseUp={handleTouchEnd} onMouseLeave={handleTouchEnd}><div className="flex justify-between items-start mb-2"><div><h3 className="font-bold text-white text-lg">{item.borrower_name}</h3><p className="text-xs text-neutral-500">Lent on {formatDate(item.lent_date)}</p></div><div className="text-right"><span className={`font-bold text-lg ${item.is_returned ? 'text-green-500 line-through' : 'text-red-500'}`}>{formatMoney(item.amount, currentUser.currency, privacy)}</span></div></div>{item.attachment_lent && (<div className="mt-2 mb-2 p-2 bg-black/30 rounded-lg border border-white/5 flex items-center gap-2 cursor-pointer hover:bg-black/50" onClick={(e) => { e.stopPropagation(); setViewAtt(item.attachment_lent); }}><Eye size={14} className="text-blue-400"/> <span className="text-xs text-blue-300">View Proof</span></div>)}{!item.is_returned ? (<div className="flex justify-between items-center mt-3 pt-3 border-t border-white/5"><span className="text-xs text-neutral-500 flex items-center gap-1"><Bell size={12}/> {item.reminder_date ? `Remind: ${formatDate(item.reminder_date)}` : 'No reminder'}</span><button onClick={(e) => { e.stopPropagation(); setShowReturn(item.id); }} className="bg-green-700/20 text-green-400 px-3 py-1.5 rounded-lg text-xs font-bold border border-green-700/50 hover:bg-green-700/30">Mark Returned</button></div>) : (<div className="mt-2 text-xs text-green-500 flex items-center gap-1"><CheckCircle size={12}/> Returned on {formatDate(item.returned_date)}{item.attachment_returned && (<button onClick={(e) => { e.stopPropagation(); setViewAtt(item.attachment_returned); }} className="ml-2 hover:text-green-300"><Eye size={12}/></button>)}</div>)}</div>))}</div>
            {(showAdd || showEdit) && (<Modal title={showEdit ? "Edit Lending" : "Lend Money"} onClose={() => { setShowAdd(false); setShowEdit(false); setLendOptions(null); }}><form onSubmit={handleAddOrUpdate} className="space-y-6"><FormField label="Friend Name"><Input value={newItem.borrower_name} onChange={e=>setNewItem({...newItem, borrower_name: e.target.value})} required/></FormField><FormField label="Amount"><Input type="number" value={newItem.amount} onChange={e=>setNewItem({...newItem, amount: e.target.value})} required/></FormField><div className="grid grid-cols-2 gap-4"><FormField label="Lent Date"><Input type="date" value={newItem.lent_date} onChange={e=>setNewItem({...newItem, lent_date: e.target.value})} required/></FormField><FormField label="Reminder"><Input type="date" value={newItem.reminder_date} onChange={e=>setNewItem({...newItem, reminder_date: e.target.value})}/></FormField></div><FormField label="Proof"><div className="border-2 border-dashed border-neutral-700 rounded-xl p-4 text-center cursor-pointer hover:bg-neutral-800" onClick={() => fileRef.current.click()}><Upload className="mx-auto text-neutral-500 mb-2"/><span className="text-xs text-neutral-400">{newItem.attachment_lent ? 'File Selected' : 'Upload'}</span></div><input type="file" ref={fileRef} accept="image/*,.pdf" className="hidden" onChange={(e) => handleFile(e, setNewItem, 'attachment_lent')}/></FormField><button className="w-full bg-red-600 text-white py-3.5 rounded-xl font-bold mt-2">Save</button></form></Modal>)}
            {showReturn && <Modal title="Mark as Returned" onClose={() => setShowReturn(null)}><form onSubmit={handleReturn} className="space-y-6"><FormField label="Returned Date"><Input type="date" value={returnItem.returned_date} onChange={e=>setReturnItem({...returnItem, returned_date: e.target.value})} required/></FormField><FormField label="Proof"><div className="border-2 border-dashed border-neutral-700 rounded-xl p-4 text-center cursor-pointer hover:bg-neutral-800" onClick={() => returnFileRef.current.click()}><Upload className="mx-auto text-neutral-500 mb-2"/><span className="text-xs text-neutral-400">{returnItem.attachment_returned ? 'File Selected' : 'Upload'}</span></div><input type="file" ref={returnFileRef} accept="image/*,.pdf" className="hidden" onChange={(e) => handleFile(e, setReturnItem, 'attachment_returned')}/></FormField><button className="w-full bg-green-600 text-white py-3.5 rounded-xl font-bold mt-2">Confirm</button></form></Modal>}
            {viewAtt && <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/95 p-4" onClick={() => setViewAtt(null)}><button onClick={() => setViewAtt(null)} className="absolute top-6 right-6 bg-neutral-800 text-white p-3 rounded-full z-[80]"><X/></button>{viewAtt.startsWith('data:application/pdf') ? <iframe src={viewAtt} className="w-full h-[85vh] rounded-lg border-none"/> : <img src={viewAtt} className="max-w-full max-h-[85vh] object-contain"/>}</div>}
        </div>
    );
};
...
const IncomePage = ({ currentUser, privacy }) => {
    ...
    const handleDeleteComp = async (id) => { if(!confirm("Delete?")) return; const token = localStorage.getItem('token'); try { await axios.delete(`${API_URL}/income/companies/${id}`, { headers: { Authorization: `Bearer ${token}` } }); setCompOptions(null); fetchData(); } catch(err) { alert("Failed"); } };
    
    const handleLogOrUpdateSal = async (e) => { 
        e.preventDefault(); 
        if(!newSal.company_id) { alert("Select company"); return; } 
        const token = localStorage.getItem('token'); 
        try { 
            const payload = { amount: parseFloat(newSal.amount), date: new Date(newSal.date).toISOString(), company_id: parseInt(newSal.company_id), slip: newSal.slip || null }; 
            if (showEditSal && salOptions) { await axios.put(`${API_URL}/income/salary/${salOptions.id}`, payload, { headers: { Authorization: `Bearer ${token}` } }); } 
            else { await axios.post(`${API_URL}/income/salary`, payload, { headers: { Authorization: `Bearer ${token}` } }); } 
    ...
    const handleDeleteSal = async (id) => { if(!confirm("Delete?")) return; const token = localStorage.getItem('token'); try { await axios.delete(`${API_URL}/income/salary/${id}`, { headers: { Authorization: `Bearer ${token}` } }); setSalOptions(null); fetchData(); } catch(err) { alert("Failed"); } };
    const handleLogoUpload = async (e) => { try { const b64 = await processImage(e.target.files[0]); setNewComp(prev => ({...prev, logo: b64})); } catch(err) { alert("Error"); } };
    const handleSlipUpload = async (e) => { try { const b64 = await processImage(e.target.files[0]); setNewSal(prev => ({...prev, slip: b64})); } catch(err) { alert("Error"); } };
    ...
    return (
        <div className="space-y-8 animate-in fade-in relative">
            {/* Comp Options */}
            {compOptions && !showEditComp && (<div className="fixed inset-0 z-50 flex items-center justify-center animate-in fade-in duration-200"><div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => setCompOptions(null)}></div><div className="bg-neutral-900 border border-neutral-700 p-1 rounded-2xl w-3/4 max-w-[200px] flex flex-col gap-1 z-50"><div className="text-center py-3 text-xs font-bold text-neutral-400 uppercase border-b border-neutral-800">Options</div><button onClick={() => startEditComp(compOptions)} className="w-full bg-neutral-800 text-white p-3 rounded-xl flex items-center gap-3 hover:bg-neutral-700"><Edit2 size={16}/>Edit</button><button onClick={() => handleDeleteComp(compOptions.id)} className="w-full bg-neutral-800 text-red-400 p-3 rounded-xl flex items-center gap-3 hover:bg-red-900/20"><Trash2 size={16}/>Delete</button><button onClick={() => setCompOptions(null)} className="w-full text-neutral-500 text-sm py-3">Cancel</button></div></div>)}
            {/* Sal Options */}
            {salOptions && !showEditSal && (<div className="fixed inset-0 z-50 flex items-center justify-center animate-in fade-in duration-200"><div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => setSalOptions(null)}></div><div className="bg-neutral-900 border border-neutral-700 p-1 rounded-2xl w-3/4 max-w-[200px] flex flex-col gap-1 z-50"><div className="text-center py-3 text-xs font-bold text-neutral-400 uppercase border-b border-neutral-800">Options</div><button onClick={() => startEditSal(salOptions)} className="w-full bg-neutral-800 text-white p-3 rounded-xl flex items-center gap-3 hover:bg-neutral-700"><Edit2 size={16}/>Edit</button><button onClick={() => handleDeleteSal(salOptions.id)} className="w-full bg-neutral-800 text-red-400 p-3 rounded-xl flex items-center gap-3 hover:bg-red-900/20"><Trash2 size={16}/>Delete</button><button onClick={() => setSalOptions(null)} className="w-full text-neutral-500 text-sm py-3">Cancel</button></div></div>)}
            
            <div className="flex justify-between items-center"><h2 className="text-2xl font-bold text-white">Income Streams</h2><div className="flex gap-2"><button onClick={() => { setNewComp({ name: '', joining_date: new Date().toISOString().split('T')[0], is_current: true, logo: '' }); setShowAddComp(true); }} className="bg-neutral-800 text-white p-2 rounded-lg border border-neutral-700"><Briefcase size={20}/></button><button onClick={() => { setNewSal({ amount: '', date: new Date().toISOString().split('T')[0], company_id: '', slip: '' }); setShowLogSal(true); }} className="bg-green-700 text-white p-2 rounded-lg"><Plus size={20}/></button></div></div>
            <div className="flex gap-3 overflow-x-auto pb-2 no-scrollbar">{companies.map(c => (<div key={c.id} onClick={() => setViewCompany(c)} className="min-w-[150px] bg-neutral-900 border border-neutral-800 p-4 rounded-xl flex flex-col justify-between h-28 relative select-none transition-transform active:scale-95 cursor-pointer" onTouchStart={() => handleTouchStart(c, setCompOptions)} onTouchEnd={handleTouchEnd} onMouseDown={() => handleTouchStart(c, setCompOptions)} onMouseUp={handleTouchEnd} onMouseLeave={handleTouchEnd}>{c.logo ? (<img src={c.logo} className="w-8 h-8 object-contain rounded mb-1" alt="logo" />) : (<Briefcase size={24} className="text-blue-500 mb-1"/>)}<div><p className="font-bold text-white text-sm truncate">{c.name}</p><p className="text-xs font-bold text-green-500 mt-1">{formatMoney(getCompanyTotal(c.id), currentUser.currency, privacy)}</p></div>{c.is_current && <div className="absolute top-2 right-2 w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>}</div>))}</div>
            ...
            {viewingSlip && <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/95 p-4" onClick={() => setViewingSlip(null)}><button onClick={() => setViewingSlip(null)} className="absolute top-6 right-6 bg-neutral-800 text-white p-3 rounded-full z-[80]"><X/></button>{viewingSlip.startsWith('data:application/pdf') ? <iframe src={viewingSlip} className="w-full h-[85vh] rounded-lg border-none"/> : <img src={viewingSlip} className="max-w-full max-h-[85vh] object-contain"/>}</div>}
        </div>
    );
};
