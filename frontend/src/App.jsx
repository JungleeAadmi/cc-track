...
const handleError = (err, context = "Action") => {
    ...
    if (err.response && (err.response.status === 413 || err.response.status === 431)) {
        alert(`${context} Failed: File too large. Max limit is ~10MB.`);
    } else {
...
const processImage = (file) => {
  return new Promise((resolve, reject) => {
    ...
    if (file.size > 10 * 1024 * 1024) {
        return reject("File too large. Please select a file under 10MB.");
    }
...
const TransactionsModal = ({ onClose, currency }) => {
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            await axios.put(`${API_URL}/transactions/${editingTxn.id}`, { description: editingTxn.description, amount: parseFloat(editingTxn.amount), date: new Date(editingTxn.date).toISOString() }, { headers: { Authorization: `Bearer ${token}` } });
            setEditingTxn(null); fetchTxns();
...
const EditCardModal = ({ card, onClose, onDelete, onUpdate }) => {
  ...
  const handleUpdateCard = async () => {
      const token = localStorage.getItem('token');
      try {
          const payload = { ...formData, total_limit: parseFloat(formData.total_limit) || 0, manual_limit: formData.manual_limit ? parseFloat(formData.manual_limit) : null, statement_date: parseInt(formData.statement_date) || 1, payment_due_date: parseInt(formData.payment_due_date) || 1 };
  ...
  const handleAddOrUpdateStatement = async (e) => {
    ...
    try {
        ...
        if (editingStmtId) { await axios.put(`${API_URL}/cards/${card.id}/statements/${editingStmtId}`, payload, { headers: { Authorization: `Bearer ${token}` } }); } else { await axios.post(`${API_URL}/cards/${card.id}/statements`, payload, { headers: { Authorization: `Bearer ${token}` } }); }
  ...
  const toggleStatementPaid = async (stmt) => {
      const token = localStorage.getItem('token');
      try { await axios.put(`${API_URL}/cards/${card.id}/statements/${stmt.id}`, { is_paid: !stmt.is_paid }, { headers: { Authorization: `Bearer ${token}` } }); fetchStatements(); } catch(err) { alert("Failed to update status"); }
  };

  const handleDeleteStatement = async (stmtId) => {
      if(!confirm("Delete this statement?")) return; const token = localStorage.getItem('token');
      try { await axios.delete(`${API_URL}/cards/${card.id}/statements/${stmtId}`, { headers: { Authorization: `Bearer ${token}` } }); fetchStatements(); setStmtOptions(null); } catch(err) { alert("Failed to delete statement"); }
  };

  const startEditStatement = (stmt) => {
      let dateStr = new Date().toISOString().split('T')[0];
      if (stmt.date) dateStr = new Date(stmt.date).toISOString().split('T')[0];
      setNewStmt({ date: dateStr, amount: stmt.amount }); setEditingStmtId(stmt.id); setStmtOptions(null);
