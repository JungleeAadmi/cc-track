...
const SubscriptionsPage = ({ currency, privacy }) => {
    ...
    const handleAddOrUpdate = async (e) => {
        e.preventDefault(); 
        if(!newSub.amount || isNaN(parseFloat(newSub.amount))) { alert("Invalid amount"); return; }
        const token = localStorage.getItem('token');
...
const LendingPage = ({ currentUser, privacy }) => {
    ...
    // FIXED STATE INIT to match inputs
    ...
    const handleAddOrUpdate = async (e) => { 
        e.preventDefault(); 
        if(!newItem.amount || isNaN(parseFloat(newItem.amount))) { alert("Invalid amount"); return; }
        const token = localStorage.getItem('token'); 
        try { 
    ...
    const handleReturn = async (e) => { 
        e.preventDefault(); 
        if(!returnItem.amount || isNaN(parseFloat(returnItem.amount))) { alert("Invalid amount"); return; }
        const token = localStorage.getItem('token'); 
        try { 
