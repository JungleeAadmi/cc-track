  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <div className="space-y-2">
        {subs.map(sub => (
          <div key={sub.id} className="bg-surface border border-white/5 p-3 rounded-xl flex justify-between items-center hover:bg-white/5 transition-colors">
            ...
            <div className="flex items-center gap-4">
                <p className="font-bold text-slate-200"><Money amount={sub.amount}/></p>
        ...
        {subs.length > 0 && <div className="flex justify-between px-4 py-2 border-t border-white/10 mt-4"><span className="text-slate-400">Total Monthly:</span><span className="font-bold text-white"><Money amount={subs.reduce((acc, curr) => acc + curr.amount, 0)}/></span></div>}
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                  ...
                  <select className="w-full h-12 bg-black/40 border border-slate-700 rounded-xl px-4 text-white" value={form.frequency} onChange={e=>setForm({...form, frequency: e.target.value})}>
