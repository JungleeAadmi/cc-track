...
import { Plus, RotateCw, CheckCircle, FileText } from 'lucide-react';
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-6">
                <div className="perspective-1000 w-full aspect-[1.58/1] cursor-pointer group" onClick={() => setIsFlipped(!isFlipped)}>
                    <div className={`relative w-full h-full transition-transform duration-700 transform-style-3d ${isFlipped ? 'rotate-y-180' : ''}`}>
                        <div className="absolute inset-0 backface-hidden"><VirtualCard card={card} isMasked={false} /></div>
                        <div className="absolute inset-0 backface-hidden rotate-y-180 h-full w-full bg-slate-900 rounded-2xl border border-white/20 flex flex-col shadow-2xl overflow-hidden relative">
                             {card.back_image_path ? (
                                <img src={`/uploads/${card.back_image_path}`} className="absolute inset-0 w-full h-full object-cover opacity-80" />
                             ) : (
                                <div className="absolute inset-0 flex items-center justify-center text-slate-500">No Back Image</div>
                             )}
                             ...
                             <div className="w-full px-8 mt-4 relative z-10">
                                ...
                                <div className="text-[10px] text-white/80 mt-1 text-right mr-1 drop-shadow-md font-bold">CVV</div>
                            </div>
                        </div>
                    </div>
                </div>
...
const Cards = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if(isEditing) await api.put(`/api/cards/${actionCard.id}`, formData);
      else await api.post('/api/cards/', formData);
      setShowAddModal(false); setForm(initialForm); setFrontImg(null); setBackImg(null); setIsEditing(false); setActionCard(null);
      fetchCards();
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {cards.map(card => (
            <div key={card.id} data-card-id={card.id} {...longPressProps} className="relative touch-manipulation">
                <VirtualCard card={card} isMasked={true} />
            </div>
        ))}
      </div>

      <CardDetailModal card={selectedCard} isOpen={!!selectedCard} onClose={()=>setSelectedCard(null)} onRefresh={fetchCards}/>
      <ActionMenu isOpen={!!actionCard} onClose={()=>setActionCard(null)} onDelete={handleDelete} onEdit={handleEdit} />
