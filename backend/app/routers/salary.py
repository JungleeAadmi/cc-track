...
# ... Companies GET/POST/PUT/DELETE (Keep existing) ...
...
def get_companies(current_user: models.User = Depends(auth.get_current_user), db: Session = Depends(database.get_db)):
    return db.query(models.Company).filter(models.Company.owner_id == current_user.id).order_by(models.Company.joining_date.desc()).all()

@router.post("/companies")
async def add_company(name: str = Form(...), joining_date: str = Form(...), relieving_date: str = Form(None), is_current: bool = Form(False), logo: UploadFile = File(None), current_user: models.User = Depends(auth.get_current_user), db: Session = Depends(database.get_db)):
    logo_path = None
    if logo:
    ...
    new_comp = models.Company(owner_id=current_user.id, name=name, joining_date=j_date, relieving_date=r_date, is_current=is_current, logo_path=logo_path)
    db.add(new_comp)
    db.commit()
    db.refresh(new_comp)
    return new_comp
...
async def update_company(company_id: int, name: str = Form(...), joining_date: str = Form(...), relieving_date: str = Form(None), is_current: bool = Form(False), logo: UploadFile = File(None), current_user: models.User = Depends(auth.get_current_user), db: Session = Depends(database.get_db)):
...
@router.post("/slips")
async def add_salary(company_id: int = Form(...), amount: float = Form(...), month: str = Form(...), year: int = Form(...), slip: UploadFile = File(None), current_user: models.User = Depends(auth.get_current_user), db: Session = Depends(database.get_db)):
    comp = db.query(models.Company).filter(models.Company.id == company_id, models.Company.owner_id == current_user.id).first()
    if not comp: raise HTTPException(status_code=404, detail="Company not found")
    file_path = None
    if slip:
